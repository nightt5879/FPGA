...
localparam
    ...
    ADDR_X_DATA_1    = 6'h20,
    ADDR_X_CTRL      = 6'h24,
    ADDR_LEN_DATA_0  = 6'h28,
    ADDR_LEN_CTRL    = 6'h2c,
    ADDR_COEF_DATA_0 = 6'h30,
    ADDR_COEF_DATA_1 = 6'h34,
    ADDR_COEF_CTRL   = 6'h38,
    WRIDLE           = 2'd0,
    WRDATA           = 2'd1,
    WRRESP           = 2'd2,
    WRRESET          = 2'd3,
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_COEF_DATA_0: begin
                    rdata <= int_coef[31:0];
                end
                ADDR_COEF_DATA_1: begin
                    rdata <= int_coef[63:32];
                end
            endcase
        end
    end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_COEF_DATA_0)
            int_coef[31:0] <= (WDATA[31:0] & wmask) | (int_coef[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_COEF_DATA_1)
            int_coef[63:32] <= (WDATA[31:0] & wmask) | (int_coef[63:32] & ~wmask);
    end
end
