...
end entity fir_wrap_CTRL_s_axi;
...
architecture behave of fir_wrap_CTRL_s_axi is
    ...
    constant ADDR_LEN_DATA_0  : INTEGER := 16#28#;
    constant ADDR_LEN_CTRL    : INTEGER := 16#2c#;
    constant ADDR_COEF_DATA_0 : INTEGER := 16#30#;
    constant ADDR_COEF_DATA_1 : INTEGER := 16#34#;
    constant ADDR_COEF_CTRL   : INTEGER := 16#38#;
    ...
    signal int_coef            : UNSIGNED(63 downto 0) := (others => '0');
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_COEF_DATA_0 =>
                        rdata_data <= RESIZE(int_coef(31 downto 0), 32);
                    when ADDR_COEF_DATA_1 =>
                        rdata_data <= RESIZE(int_coef(63 downto 32), 32);
                    when others =>
                        NULL;
                    end case;
                end if;
...
-- ----------------------- Register logic ----------------
    ...
    coef                 <= STD_LOGIC_VECTOR(int_coef);
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                int_coef(31 downto 0) <= (others => '0');
            elsif (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_COEF_DATA_0) then
                    int_coef(31 downto 0) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_coef(31 downto 0));
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                int_coef(63 downto 32) <= (others => '0');
            elsif (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_COEF_DATA_1) then
                    int_coef(63 downto 32) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_coef(63 downto 32));
                end if;
            end if;
        end if;
    end process;
